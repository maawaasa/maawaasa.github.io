...
# Team test user (dynamic — نفس نمط smoke-payment-approve المجرَّب 13/13)
TS=$(date +%s)
TM_EMAIL="e2e-tm-$TS@test.local"; TM_PASS="E2e-${TS}-Zq7!"
TM_UID=$(curl -s -X POST "$BASE/auth/v1/admin/users" "${SH[@]}" -H "Content-Type: application/json" \
  -d "{\"email\":\"$TM_EMAIL\",\"password\":\"$TM_PASS\",\"email_confirm\":true}" \
  | jq -r '.id // ""')
[ -n "$TM_UID" ] || { echo "FAIL: team user create"; exit 1; }
curl -s -o /dev/null -X POST "$SB/employees" "${SH[@]}" -H "Content-Type: application/json" \
  -d "{\"auth_user_id\":\"$TM_UID\",\"name\":\"E2E Smoke TM\",\"role\":\"admin\",\"is_active\":true}"
...
TEAM=(-H "apikey: $ANON" -H "Authorization: Bearer $JWT")
...
Q_BODY=$(jq -n \
  ...
  '{p_full_name:$name, p_phone:$phone, p_email:$email,
    p_property_type:"apartment", p_service_type:"تصوير HDR — فيديو سينمائي",
    p_total:2000,
    p_units:[{kind:"main",type:"apartment",rooms:2,qty:1}],
    p_payload:{shoot_date:$date, start:"10:00", end:"12:00",
               notes:"[MAAWAA E2E TEST — IGNORE]"}}')
...
B_BODY=$(jq -n \
  ...
  --arg loc "الرياض — حي النرجس" \
  '{p_full_name:$name, p_phone:$phone, p_email:$email,
    p_service_type:"تصوير HDR — فيديو سينمائي", p_total:2000,
    p_property_type:"apartment", p_property_location:$loc,
    p_shoot_date:$date, p_shoot_start_time:"10:00", p_shoot_end_time:"12:00",
    p_rooms:"2", p_quote_number:$qnum,
    p_units:[{kind:"main",type:"apartment",rooms:2,qty:1}],
    p_notes:"[MAAWAA E2E TEST — IGNORE] story"}')
...
c=$(curl -s -o /tmp/e2e_cc.json -w "%{http_code}" -X POST "$FN/coverage-confirm" \
  "${TEAM[@]}" -H "Content-Type: application/json" \
...
HOLD=$(api "contracts?select=hold_started_at,hold_expires_at,deposit_review_status&id=eq.$B_ID" | jq '.[0]')
...
C04=$(api "email_log?select=id,recipient,status&entity_id=eq.$B_ID&event_type=eq.C-04" | jq '[.[] | select(.status == "sent" and .recipient == "wfalfaifi@gmail.com")] | length')
...
IMG_JSON=$(jq -Rs . /tmp/e2e_receipt.txt)
RSLT=$(jq -n --arg token "$DEP_TOK" --argjson img "$IMG_JSON" \
  '{token:$token, image_data_url:$img}' \
  | curl -s -w "\n%{http_code}" -X POST "$FN/receipt-submit" \
    -H "Content-Type: application/json" --data @-)
R_CODE=$(echo "$RSLT" | tail -1)
R_BODY=$(printf '%s' "$RSLT" | sed '$d')
...
POST_R=$(api "contracts?select=hold_expires_at,deposit_review_status,receipt_image_url&id=eq.$B_ID" | jq '.[0]')
HE_AFTER=$(echo "$POST_R" | jq -r '.hold_expires_at // "NULL"')
DRS=$(echo "$POST_R" | jq -r '.deposit_review_status // ""')
if [ "$HE_AFTER" = "NULL" ]; then
  ok "hold_expires_at = NULL (expiry stopped) ✓"
else
  no "hold_expires_at = $HE_AFTER (not NULL!)"
fi
...
C08=$(api "email_log?select=id,recipient,status&entity_id=eq.$B_ID&event_type=eq.C-08" | jq '[.[] | select(.status == "sent" and .recipient == "wfalfaifi@gmail.com")] | length')
...
DISC=$(api "contracts?select=total_amount&id=eq.$B_ID" | jq -r '.[0].total_amount')
[ "$DISC" = "2000.00" ] && ok "total unchanged = 2000.00 ✓" || no "total = $DISC"
UNITS=$(api "contracts?select=units&id=eq.$B_ID" \
  | jq -r '.[0].units // "null"')
...
QID=$(api "quotes?select=id&quote_number=eq.$Q_NUM" | jq -r '.[0].id // ""')
[ -n "$QID" ] && del "quotes?id=eq.$QID"
[ -n "$TM_UID" ] && curl -s -o /dev/null -X DELETE "$SB/employees?auth_user_id=eq.$TM_UID" "${SH[@]}"
[ -n "$TM_UID" ] && curl -s -o /dev/null -X DELETE "$BASE/auth/v1/admin/users/$TM_UID" "${SH[@]}"
